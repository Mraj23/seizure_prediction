...
class CNNModel(nn.Module):
  def __init__(self):
    ...
    self.cnn1 = nn.Conv1d(in_channels=6, out_channels=21, kernel_size=3, stride=1, padding=0)
    self.relu1 = nn.ReLU() 
...
def create_thing():
    ...
    def create_power_spectra(data):
        ...
        new_data = []
        for ex in data:
            ex = np.delete(ex, [0,1,3,4,5,6,8,10,14,15,16,17,18], 0)
            new_data.append([])
            for ch in ex:
                yf = fft(ch)
